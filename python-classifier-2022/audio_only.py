...
class AudioClassifier(nn.Module):
    def __init__(self):
        ...
        self.output_murmurt_fc = nn.Linear(256, 3)

        # Concat layer for the combined feature space for outcome classification
        self.output_outcome_fc = nn.Linear(256, 2)

    def forward(self, audio_embeddings, clinical_features):
        aud_out = F.relu(self.aud_fc2(self.aud_fc1(audio_embeddings)))
        # aud_out = self.aud_fc(audio_embeddings)
        # cli_out = self.cli_fc(clinical_features)

        murmur = self.output_murmurt_fc(aud_out)
        outcome = self.output_outcome_fc(aud_out)
...
def training(model, train_dl, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(model.state_dict(), f'{audio_only_model_path}_{epoch}.sav')
        torch.save(model.state_dict(), last_audio_only_model_path)
...
with open(filtered_features_file_path, 'rb') as f:
    ...
    if os.path.exists(last_audio_only_model_path):
        model.load_state_dict(torch.load(last_audio_only_model_path))
        tqdm.write(f'Loaded model from {last_audio_only_model_path}')
