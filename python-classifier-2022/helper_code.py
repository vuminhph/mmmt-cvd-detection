...
DATA_PATH = "C:/Users/lumin/Desktop/Work/20212/Data/circor-heart-sound/final"
...
# Load recordings.
def load_recordings(data_folder, data, get_frequencies=False, get_paths=True, preprocess=True):
    ...
    for i in range(num_locations):
        ...
        file_path = os.path.join(data_folder, recording_file)
        if preprocess:
            recording, frequency = AudioUtil.load_wav_file(file_path)
        else:
            recording, frequency = AudioUtil.load_wav_file_no_preprocessing(file_path)
    ...
    if get_frequencies:
        if get_paths:
            return file_paths, recordings, frequencies
        else:
            return recordings, frequencies
    else:
        if get_paths:
            return file_paths, recordings
        else:
            return recordings
