...
from team_code_v2 import load_challenge_model, run_challenge_model
...
def run_model(model_folder, data_folder, output_folder, allow_failures, verbose):
    ...
    for i in tqdm(range(num_patient_files), desc="Patients", position=0):
        ...
        recording_paths, recordings = load_recordings(data_folder, patient_data, get_paths=True)

        # Allow or disallow the model to fail on parts of the data; helpful for debugging.
        try:
            classes, labels, probabilities = run_challenge_model(model, patient_data, recordings, recording_paths, verbose) ### Teams: Implement this function!!!
        except:
            if allow_failures:
                if verbose >= 2:
                    print('... failed.')
